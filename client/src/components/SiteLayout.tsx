  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div
          className="flex h-16 items-center justify-between rounded-b-2xl px-4 sm:px-6"
          style={{
        ...
        >
          <Link href="/site" className="flex items-center gap-2.5 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/">
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-medium text-primary/80 hover:text-primary hover:bg-primary/8 transition-all duration-200">
        ...
        {mobileOpen && (
          ...
          >
            <div className="p-3 space-y-0.5">
                  ...
                  <Link key={cat.key} href={cat.path}>
                    <span
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
                        isActive ? "text-primary bg-primary/8 font-medium" : "text-muted-foreground hover:text-foreground hover:bg-accent"
                      }`}
              ...
              <Link href="/site/about">
                <span className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-muted-foreground hover:text-foreground hover:bg-accent transition-all" onClick={() => setMobileOpen(false)}>
                  <Info className="h-4 w-4 shrink-0" />
                  关于我们
                </span>
              </Link>
              <Link href="/">
                <span className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-primary/80 hover:text-primary hover:bg-primary/8 transition-all" onClick={() => setMobileOpen(false)}>
...
function SiteFooter() {
  ...
  return (
    <footer className="border-t border-border/40 mt-20 bg-secondary/30">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            <h4 className="text-muted-foreground/60 text-xs font-semibold uppercase tracking-wider mb-4">内容板块</h4>
            <div className="space-y-2.5">
              {categories.slice(0, half).map((cat) => (
                <Link key={cat.key} href={cat.path}>
                  <span className="block text-muted-foreground hover:text-foreground text-sm transition-colors">{cat.label}</span>
                </Link>
              ))}
            </div>
          </div>
          <div>
            <h4 className="text-muted-foreground/60 text-xs font-semibold uppercase tracking-wider mb-4">更多</h4>
            <div className="space-y-2.5">
              ...
              <Link href="/site/about">
                <span className="block text-muted-foreground hover:text-foreground text-sm transition-colors">关于我们</span>
              </Link>
            </div>
          </div>
          <div>
            <h4 className="text-muted-foreground/60 text-xs font-semibold uppercase tracking-wider mb-4">联系</h4>
